...
internal class XMLParser {
     ...
     */
    const String PREFS_DTD_NAME = "http://java.sun.com/dtd/preferences.dtd"; //$NON-NLS-1$
     ...
     */
    const String HEADER = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"; //$NON-NLS-1$

    /*
     * Constant - the specified DOCTYPE
     */
    const String DOCTYPE = "<!DOCTYPE preferences SYSTEM"; //$NON-NLS-1$
     ...
     * of Linux platform
     */
    private const String FILE_PREFS = "<!DOCTYPE map SYSTEM 'http://java.sun.com/dtd/preferences.dtd'>"; //$NON-NLS-1$
    ...
    internal static void exportPrefs(Preferences prefs, java.io.OutputStream stream,
            bool withSubTree) {//throws IOException, BackingStoreException {
        indent = -1;
    ...
    private static void exportEntries(String[] keys, String[] values,
            java.io.BufferedWriter outJ){// throws IOException {
        if (keys.Length == 0) {
            flushEmptyElement("map", outJ); //$NON-NLS-1$
            return;
        }
    ...
    private static void flushEmptyElement(String tagName, String[] attrKeys,
            String[] attrValues, java.io.BufferedWriter outJ){// throws IOException {
        flushIndent(++indent, outJ);
        outJ.write("<"); //$NON-NLS-1$
        outJ.write(tagName);
