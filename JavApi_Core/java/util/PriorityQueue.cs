 ...
 */
	[Serializable]
	public class PriorityQueue<E> : AbstractQueue<E>, java.io.Serializable {

    private const long serialVersionUID = -7720805057305804111L;

    private const int DEFAULT_CAPACITY = 11;

    private const double DEFAULT_INIT_CAPACITY_RATIO = 1.1;

    private const int DEFAULT_CAPACITY_RATIO = 2;
    ...
    public PriorityQueue(Collection<E> c) {
        if (c is PriorityQueue) {
            getFromPriorityQueue((PriorityQueue<E>) c);
        } else if (c is SortedSet) {
            getFromSortedSet((SortedSet<E>) c);
        } else {
            initSize(c);
            addAll(c);
        }
     ...
     */
		...
		private T[] newArray <T>(T[] array) {
        if (size > array.Length) {
            Class clazz = array.getClass().getComponentType();
            array = (T[]) Array.newInstance(clazz, size);
        }
        java.lang.SystemJ.arraycopy(elements, 0, array, 0, size);
        if (size < array.Length) {
            array[size] = null;
        }
        return array;
    ...
    private class PriorityIterator : Iterator<E> {
    ...
    private void getFromPriorityQueue(PriorityQueue<E> c) {
        initSize(c);
    ...
    private void getFromSortedSet(SortedSet<E> c) {
        initSize(c);
        comparator = (Comparator<? super E>) c.comparator();
    ...
    private void initSize(Collection<E> c) {
        if (null == c) {
            throw new NullPointerException();
        }
        if (c.isEmpty()) {
